...
A real database walks in like an uninvited guest: installation, connection strings, migrations, connection pooling, async drivers — all before a single line of domain logic is written. For now, the restaurant parks its data in a paper notebook on the kitchen counter: fast, no setup, and it evaporates on restart, which is exactly the bar "make it work" sets.
...
Each service keeps its own private notebook — no shared cellar until Day 3.
...
- On Day 3 the notebook is **completely replaced** with the cellar — PostgreSQL + Drizzle/asyncpg

**Deprecation Schedule**:
- Day 3: the notebook is swapped for the cellar — PostgreSQL with a proper schema (3 tables)
- Day 3: `Map`/`dict` lookups become Drizzle ORM / asyncpg queries
- The customer-facing counter stays — only the kitchen behind it changes (service API surface unchanged)
