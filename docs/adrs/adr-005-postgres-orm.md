...
Day 3 of Sprint 1 burns the in-memory notebook and moves the data into the restaurant's cellar — PostgreSQL with proper shelves, per the roadmap. The dual-language track means two foremen for the same cellar: one ORM per language, each with its own ecosystem, async model, and community conventions.

The decision has two axes: which database, and which ORM/driver per track.

## Decision

- **Database**: PostgreSQL 15 — one cellar, shared by both tracks, run in a room (Docker Compose) with `postgres:postgres` credentials on port 5432
...
Both tracks walk into the same cellar with the same key — same schema, same data, same connection string. Drizzle stays close to the stone (SQL), SQLAlchemy brings full-renovator machinery (ORM mapping, sessions). The difference is the client-side abstraction, the *client-side* only.

### Schema Strategy (temporary)

- **TS**: Drizzle `db:push` — the TypeScript definitions *are* the blueprint, pushed straight to the cellar (no drawings kept on file — the code is the record)
- **Python**: SQLAlchemy `init_db()` with `Base.metadata.create_all()` — the cellar is inspected at app startup and missing shelves are built, but existing shelves are never remodeled
...
**Positive**:
- One database to learn — PostgreSQL is the roadmap's long-term choice; the foremen argue over the blueprint, but they agreed the *cellar* stays
...
- SQLAlchemy async is production-proven — the kitchen style a decade of FastAPI built on
...
- Connection pooling is implicit (the drivers run their own pool) — less control over pool size today; headroom while one laptop runs the restaurant, a moving target once it is shared

**Mitigation**:
- Three tables is disposable — while the cellar holds only 3 shelves, the sledgehammer (demolish + rebuild the cellar) is acceptable; the rule until Alembic lands: change a track, pour a new cellar
...
- The repository pattern keeps the domain out of the cellar — a foreman swap touches `repository.ts`/`repository.py` only
- The database stays local-only (`127.0.0.1:5432`) — Deepening 4 (Weeks 15-18) is where the restaurant moves onto the network; then only the doorknob (the connection string, already an environment variable on both tracks) changes
