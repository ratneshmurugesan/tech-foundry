# Foundry Roadmap — Ship Thin, Deepen Later
...
**The rule**: Get something broken but working in **2 weeks**. Deploy it in **week 3**. Then go back and make it good through iterative deepening passes. Never spend more than 2 weeks on foundations before shipping.

```
Build thin → Deploy (week 3) → Deepen → Build more → Deepen more → ...
```

**This is how all 12 OSS repos actually grew:**
- Ghost didn't start with perfect i18n — they added it after having a working blog
- Plane didn't start with 100% test coverage — they added tests as features grew
- OpenHands didn't start with mock-LLM E2E — they added it when AI agents got complex
- Discourse didn't start with 100+ tables — they started with core forum, then expanded

**Rule**: Look at an OSS pattern for 30 minutes, build the 80% version in 20% of the time, then iterate.

**The Spiral Model**:
```
        ┌─────────────────────────────────┐
        │  Deepening Pass 5: Advanced     │  ← Event sourcing, CQRS, plugins, AI
        │  (Weeks 29-36)                  │
        └─────────────────────────────────┘
                     ▲
        ┌─────────────────────────────────┐
        │  Deepening Pass 4: Production   │  ← EKS, observability, security, DR
        │  (Weeks 21-28)                  │
        └─────────────────────────────────┘
                     ▲
        ┌─────────────────────────────────┐
        │  Deepening Pass 3: Distributed  │  ← Microservices, SQS, Sagas, resilience
        │  (Weeks 15-20)                  │
        └─────────────────────────────────┘
                     ▲
        ┌─────────────────────────────────┐
        │  Deepening Pass 2: Infrastructure│ ← K3s, Helm, Traefik, GitOps
        │  (Weeks 11-14)                  │
        └─────────────────────────────────┘
                     ▲
        ┌─────────────────────────────────┐
        │  Deepening Pass 1: Quality      │  ← Tests, DDD, multi-tenancy, observability basics
        │  (Weeks 7-10)                   │
        └─────────────────────────────────┘
                     ▲
        ┌─────────────────────────────────┐
        │  Build Sprint 3: Frontend       │  ← Next.js UI, keyboard shortcuts
        │  (Weeks 5-6)                    │
        └─────────────────────────────────┘
                     ▲
        ┌─────────────────────────────────┐
        │  Build Sprint 2: Auth + Pay     │  ← Auth0, Razorpay
        │  (Weeks 3-4)                    │
        └─────────────────────────────────┘
                     ▲
        ┌─────────────────────────────────┐
        │  Build Sprint 1: Working API    │  ← CRUD, PostgreSQL, Docker, deploy
        │  (Weeks 1-2)                    │
        └─────────────────────────────────┘
```

**By week 3, you have something deployed. By week 6, you have a real product. By week 10, it's quality. By month 9, it's production-grade.**

**Total timeline**: ~36 weeks (~9 months). Learning from real usage, not just reading.
...
**Approach**: Use the DDD concept when you need it, learn it then. No upfront theory.
...
**Each phase has a different discipline.** Apply the right one for the phase you're in.
...
**The goal**: *working code*, not type system mastery. One day is enough. You'll deepen types in Deepening 1.
...
**The verification bar changes with each phase.**
...
*End of roadmap. Ship thin, deepen later.*
