...
> **Purpose**: Master prompt for daily execution. Say "Read `tech-foundry/docs/prompts/2026-08-12-0000.md`" to restore full context.
...
3. **Never write actual code** by yourself unless explicitly asked for it.
...
8. **Add deprecation/upgrade notes** where relevant so I know what changes in Phase 2:
...
12. **After completing each day, log notes** in Markdown format at `tech-foundry/docs/notes/day-XX-[topic]-[current-timestamp].md` capturing:
...
> "Read `tech-foundry/docs/prompts/2026-08-12-0000.md` and `tech-foundry/docs/roadmaps/v4/foundry-roadmap-v4-2026-08-11-1840.md`, then give me Day [N]."

The AI will:
1. Load this prompt (rules)
2. Load the roadmap (context, phase, sprint, day definition)
3. Generate the complete day breakdown following all 11 rules
...
├── Day-01-types-and-async.md
├── Day-02-api-skeleton.md
├── Day-03-postgres-schema.md
└── ...
```

Each file follows the template in Rule 11. ADRs live separately in `tech-foundry/docs/adrs/` and are referenced by number (e.g., "per ADR-001") when relevant.
