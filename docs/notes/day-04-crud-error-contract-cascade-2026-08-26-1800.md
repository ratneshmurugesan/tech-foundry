...
| `delete_projects` 0-deleted branch raised raw `Exception` → generic 500 instead of 404 | `py/src/server.py` | ✅ Fixed → `NotFoundError` |
| `create_projects` had no workspace parent check (a bad `workspace_id` 500s on FK blow-up, not 404) | `py/src/server.py` | ✅ Fixed — explicit lookup → 404 |
| `patch_projects` never validated a *new* `workspace_id` | `py/src/server.py` | ✅ Fixed — validates destination only when present in body |
