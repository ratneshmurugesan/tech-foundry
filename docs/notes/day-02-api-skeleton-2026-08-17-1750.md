...
The sign said OPEN but the kitchen hadn't prepped — the console line fired before the restaurant was really listening. The `startServer` function called `app.listen({ port })` without `await`. Fastify's `listen()` returns a Promise — without awaiting, the function returns immediately and the console.log fires before the server is actually listening.
**Fix**: Added `await` before `app.listen({ port })`.

### `await uvicorn.run()` invalid (Python)
Treated a waiter who stays at the table as a courier who vanishes — you can't hand a waiting task to a person who just stands there. `uvicorn.run()` is a sync blocking function that returns `None`. Putting `await` in front of it throws `TypeError: object NoneType can't be used in 'await' expression`.
...
The skeleton moves to a permanent building: we pour a concrete foundation and move the shelves from the cardboard box into it.
