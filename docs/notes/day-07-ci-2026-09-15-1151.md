...
- **ADR-010: CI runs with zero services** — *"CI ships cellar-less"*: no `services: postgres` in `ci.yml`; the no-cellar path pinned in `conftest` via IANA port 9 (discard); the 500 family as a test *welded to the gate design* (deliberately untested-with-DB in Phase 1; the cellar upgrade is Phase 2's "make it right"); ADR-008 (doorless deploy) as the *twin decision*; the live-DB smoke test and the TS `tsc`-only asymmetry recorded as the day's two *named* debts. See `docs/adrs/adr-010-ci-zero-service.md` — written once the GitHub run resolved the day's one *unknown* (run #4, both jobs green) rather than on plan.

## Commands (run by me, hidden terminal; reproducible on `dev`) (rule 12)
